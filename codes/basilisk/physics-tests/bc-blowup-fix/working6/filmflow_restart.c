...
// Convective outlet variables
scalar uout_x[], uout_y[];
double c_out = 3.0; //based on linear theory. change later.
...
//u.n[right] = neumann(0.);
//u.t[right] = neumann(0.);
u.t[right] = dirichlet(uout_x[]) ;
u.n[right] = dirichlet(uout_y[]) ;
//u.t[right] = neumann(0);
p[right] = neumann(0);
pf[right] = neumann(0); 
...
event init (t = 0)
{
  if (restore (file = "dump", list = all)) {
    ...
    boundary ({uout_x, uout_y});
  }
  else {
    fprintf (stderr, "ERROR: Could not restore dump\n");
  }
}


event convective_outlet (i++, last)
{
  foreach_boundary(right) {

    double Co = c_out*dt/Delta;

    // Safety for explicit convective update
    Co = min(Co, 1.);

    uout_x[] -= Co*(uout_x[] - u.x[-1]);
    uout_y[] -= Co*(uout_y[] - u.y[-1]);
  }
